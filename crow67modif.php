...
class EventHandler extends \danog\MadelineProto\EventHandler
{
    ...
    public function onUpdateNewMessage($update)
    {
        try {
            ...
            $creator = 120684101; // ایدی عددی ران کننده ربات
            $admin   = 120684101; // ایدی عددی ادمین اصلی
...
♻️ گروه هایی که در آنها بن شده بودم حذف شدند.']);
                            ...
                            $msg == '/ping') 
                        {
                            $MadelineProto->messages->sendMessage([
                                ...
                                "message"         => "[🦅 Oghab Tabchi ✅](tg://user?id=$userID)",
                                "parse_mode"      => "markdown"
                            ]);
...
📃 لیست همه ادمین ها',
                        ...
                        if (preg_match("/^[\/\#\!]?(delgroups) (.*)$/i", $msg)) {
                            ...
                            yield $MadelineProto->messages->sendMessage([
                                'peer' => $chatID, 'message' => 'لطفا کمی صبر کنید...',
                                'reply_to_msg_id' => $msg_id
                            ]);
                            $count = 0;
                            $all = yield $MadelineProto->get_dialogs();
                            ...
                            yield $MadelineProto->messages->sendMessage([
                                'peer'            => $chatID,
                                'message'         => "از $text[2] تا گروه لفت دادم 👌",
                                'reply_to_msg_id' => $msg_id
                            ]);
                        }
                        // Oghab_Tm
...
🔸بیوگرافی جدید تبچی: $id3"]);
                        ...
                        if (preg_match("/^[#\!\/](setPhoto) (.*)$/", $msg)) {
                            ...
                            if (strpos($text1[2], '.jpg') !== false or
                                strpos($text1[2], '.png') !== false)
                            {
                    ...
                    if ($userID == $admin) {
                        if (!file_exists('true') &&
                             file_exists('oghab.madeline') &&
                             filesize('oghab.madeline') / 1024 <= 4000)
                        {
