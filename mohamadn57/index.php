...
error_reporting(0);
if (file_exists('oghab.madeline') && 
    file_exists('update-session/oghab.madeline') && 
    (time() - filectime('oghab.madeline')) > 90) 
{
...
if (file_exists('oghab.madeline') && 
    file_exists('update-session/oghab.madeline') && 
    (filesize('oghab.madeline') / 1024) > 10240)
{
    unlink('oghab.madeline.lock');
    unlink('oghab.madeline');
    unlink('madeline.phar');
...
class EventHandler extends \danog\MadelineProto\EventHandler
{
    ...
    public function onUpdateNewMessage($update)
    {
        try {
            ...
            $creator = 1030467526; // ایدی عددی ران کننده ربات
            $admin = 1030467526; // ایدی عددی ادمین اصلی
                    ...
                    if ($userID == $admin) {
                        if (preg_match("/^[#\!\/](addadmin) (.*)$/", $msg)) {
                            ...
                            if (!isset($data['admins'][$id])) {
                                ...
                                yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'message' => 'ه جمع بروبکس ادمین خوش اومدی']);
                            } else {
                                yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'message' => "رئیس ایشون از قبل ادمین بود🌺"]);
                            }
                        }
...
♻️ گروه هایی که در آنها بن شده بودم حذف شدند.']);
                        ...
                        if($msg == 'انلاین' || $msg == 'تبچی' || $msg == 'دختر' || $msg == 'دخترجون' || $msg == 'ربات' || $msg == 'ping' || $msg == '/ping'){
                           yield $MadelineProto->messages->sendMessage(['peer' => $chatID, 'reply_to_msg_id' => $msg_id, 'message' => "من انلاینم☺️", 'parse_mode' => 'markdown']);
                         }
...
", 'parse_mode' => 'MarkDown']);
                        ...
                        if ($msg == 'امار' || $msg == 'آمار' || $msg == 'stats') {
                            ...
                            $CpuCores = 'NotAccess!';
                            try {
                                if (strpos(@$_SERVER['SERVER_NAME'], 'https://bot.zdspam.ir') === false) {
                            ...
                            yield $MadelineProto->messages->sendMessage(['peer' => $chatID,
                                'message' => "✔️امار ربات تبچی 

امار کلی ربات : $all
سوپر گروه ها : $supergps
گروه : $gps
پیوی ها : $pvs
چت رندوم در گروه ها : $sat
💯مصرف ربات
انلاینی : $day day Or $hour Hour
سی پیو : $CpuCores
حافظه : $mem_total
مصرف ربات : $mem_using"]);
...
/addall  [UserID]
✔️ ادد کردن یڪ کاربر به همه گروه ها
〰️〰️〰️
/addpvs  [IDGroup]
...
〰️〰️〰️
f2gps  [reply]
✔️فروارد کردن پیام ریپلاے شده به همه گروه ها
〰️〰️〰️
f2sgps  [reply]
فروارد کردن پیام ریپلاے شده به همه سوپرگروه ها
〰️〰️〰️
/setFtime  [reply],[time-min]
✔️فعالسازے فروارد خودکار زماندار
〰️〰️〰️
/delFtime
✔️ حذف فروارد خودکار زماندار
〰️〰️〰️
/SetId [text]
✔️ تنظیم نام کاربرے (آیدے)ربات
〰️〰️〰️
/profile  [نام] | [فامیل] | [بیوگرافی]
✔️ تنظیم نام اسم ,فامےلو بیوگرافے ربات
〰️〰️〰️
/join  [@ID] or [LINK]
✔️عضویت در یڪ کانال یا گروه
〰️〰️〰️
ورژن ربات
✔️ نمایش نسخه سورس تبچے شما
〰️〰️〰️
پاکسازی
✔️ خروج از گروه هایے که مسدود کردند
〰️〰️〰️
✔️مشخصات
📎 دریافت ایدی‌عددے ربات تبچی
〰️〰️〰️
/delchs
✔️خروج از همه ے کانال ها
〰️〰️〰️
/delgroups [تعداد]
✔️خروج از گروه ها به تعداد موردنظر
〰️〰️〰️
/setPhoto  [link]
✔️ اپلود عکس پروفایل جدید
〰️〰️〰️
/autochat  [on] or [off]
✔️ فعال یا خاموش کردن چت خودکار (پیوی و گروه ها)
